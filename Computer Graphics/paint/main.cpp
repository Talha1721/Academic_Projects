    ...
    for (int i = 0; i <= c; i++)
    {
        ...
        if (DrawingsList[i].form.compare("Line")==0)
        {
            glColor3f(DrawingsList[i].r,DrawingsList[i].g, DrawingsList[i].b);
                        ...
                        GLfloat dx = DrawingsList[i].endX - DrawingsList[i].beginX;
                        GLfloat dy = DrawingsList[i].endY - DrawingsList[i].beginY;

                        GLfloat x1 = DrawingsList[i].beginX;
                        GLfloat y1 = DrawingsList[i].beginY;
...
/*
            ...
            glClear(GL_COLOR_BUFFER_BIT);
            glBegin(GL_POINTS);
            int dx = abs(DrawingsList[i].endX - DrawingsList[i].beginX);
            int dy = abs(DrawingsList[i].endY - DrawingsList[i].beginY);
            float m = float(DrawingsList[i].endY - DrawingsList[i].beginY) / float(DrawingsList[i].endX - DrawingsList[i].beginX);
            if (m >= 0 && m <= 1)
            {
                ...
                int xn, yn, xend, yend;
                xn = DrawingsList[i].beginX;
                yn = DrawingsList[i].beginY;
                xend = DrawingsList[i].endX;
                if (DrawingsList[i].endX < DrawingsList[i].beginX)
                {
                    xn = DrawingsList[i].endX;
                    yn = DrawingsList[i].endY;
                    xend = DrawingsList[i].beginX;
                }
                d = 2 * dy - dx;
                glVertex2i(xn, yn);
                while (xn < xend)
